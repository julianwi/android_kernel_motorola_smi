...
static u16 max17042_model_mul_factor(u16 a)
{
	if (fg_conf_data) {
		/* 70% Model */
		if (fg_conf_data->table_type == 3)
			return (a * 10) / 7;
		else
			return a;
	}
...
static u16 max17042_model_div_factor(u16 a)
{
	if (fg_conf_data) {
		/* 70% Model */
		if (fg_conf_data->table_type == 3)
...
{
	int ret;
	struct max17042_config_data fg_temp_data;

	ret = max17042_read_reg(chip->client,
							MAX17042_RCOMP0);
	if (ret < 0)
		goto runtime_read_err;
	fg_temp_data.rcomp0 = ret;
	ret = max17042_read_reg(chip->client,
							MAX17042_TempCo);
	if (ret < 0)
		goto runtime_read_err;
	fg_temp_data.tempCo = ret;
	if (chip->chip_type == MAX17050) {
		ret = max17042_read_reg(chip->client,
							MAX17050_QRTbl00);
		if (ret < 0)
			goto runtime_read_err;
		fg_temp_data.qrtbl00 = ret;
		ret = max17042_read_reg(chip->client,
							MAX17050_QRTbl10);
		if (ret < 0)
			goto runtime_read_err;
		fg_temp_data.qrtbl10 = ret;
		ret = max17042_read_reg(chip->client,
							MAX17050_QRTbl20);
		if (ret < 0)
			goto runtime_read_err;
		fg_temp_data.qrtbl20 = ret;
		ret = max17042_read_reg(chip->client,
							MAX17050_QRTbl30);
		if (ret < 0)
			goto runtime_read_err;
		fg_temp_data.qrtbl30 = ret;
	}
	...
	if (ret < 0)
		goto runtime_read_err;
	fg_temp_data.full_capnom = ret;
	ret = max17042_read_reg(chip->client,
							MAX17042_FullCAP);
	if (ret < 0)
		goto runtime_read_err;
	fg_temp_data.full_cap = ret;
	if (fg_conf_data->rsense) {
		if (chip->chip_type == MAX17042)
			fg_temp_data.full_capnom = max17042_model_div_factor(
					fg_temp_data.full_capnom)
					/ fg_conf_data->rsense;
		else
			fg_temp_data.full_capnom = fg_temp_data.full_capnom
						/ fg_conf_data->rsense;

		fg_temp_data.full_cap /= fg_conf_data->rsense;
	}
	ret = max17042_read_reg(chip->client,
							MAX17042_Cycles);
	if (ret < 0)
		goto runtime_read_err;
	fg_temp_data.cycles = ret;

	fg_conf_data->rcomp0 = fg_temp_data.rcomp0;
	fg_conf_data->tempCo = fg_temp_data.tempCo;
	if (chip->chip_type == MAX17050) {
		fg_conf_data->qrtbl00 = fg_temp_data.qrtbl00;
		fg_conf_data->qrtbl10 = fg_temp_data.qrtbl10;
		fg_conf_data->qrtbl20 = fg_temp_data.qrtbl20;
		fg_conf_data->qrtbl30 = fg_temp_data.qrtbl30;
	}
	fg_conf_data->full_capnom = fg_temp_data.full_capnom;
	fg_conf_data->full_cap = fg_temp_data.full_cap;
	fg_conf_data->cycles = fg_temp_data.cycles;
...
runtime_read_err:
	...
	dev_err(&chip->client->dev, "runtime params read error:%d", ret);
	dump_fg_conf_data(chip);
}
...
static void save_runtime_params(struct max17042_chip *chip)
{
	...
	retval = max17042_read_reg(chip->client, MAX17042_STATUS);
	if (retval & STATUS_POR_BIT) {
		dev_err(&chip->client->dev, "POR set! clear init bit\n");
		/* clear the dirty byte in non-volatile memory */
		fg_conf_data->config_init = 0x0;
	...
	if (!chip->pdata->save_config_data) {
		dev_err(&chip->client->dev, "Read fail! Don't save params to NV\n");
		return ;
	}
...
static void restore_runtime_params(struct max17042_chip *chip)
{
	...
	size = sizeof(*fg_conf_data) - sizeof(fg_conf_data->cell_char_tbl);
	retval = chip->pdata->restore_config_data(chip->chip_name, fg_conf_data, size);
	if (retval < 0) {
		dev_err(&chip->client->dev, "%s failed\n", __func__);
		return ;
	}
...
static void max17042_restore_conf_data(struct max17042_chip *chip)
{
	...
	if (!chip->pdata->is_init_done && chip->pdata->restore_config_data) {
		retval = chip->pdata->restore_config_data(chip->chip_name,
...
static void max17042_evt_worker(struct work_struct *work)
{
	...
	/* Check POR status and re-initialize if needed */
	status = max17042_read_reg(chip->client, MAX17042_STATUS);
	if (status >= 0 && (status & STATUS_POR_BIT)) {
		dev_info(&chip->client->dev, "POR set, load configuration\n");
		chip->pdata->is_init_done = 0;
	}
