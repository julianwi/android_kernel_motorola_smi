...
#define UID_SIZE BATTID_STR_LEN
#define BAT_EEPROM_SIZE  (NUM_PAGES * BYTES_PER_PAGE)
#define CPYRGHT_STRING_PART_1 "COPR"
#define CPYRGHT_STRING_PART_2 "MOTOROLA E.P CHARGE ONLY"
#define MAX_CPYRT_LEN 24
#define CPYRGHT_STRING_PART_1_LEN (3*BYTES_PER_PAGE)
#define CPYRGHT_STRING_PART_2_LEN ((3*BYTES_PER_PAGE) + 8)

#define MAX17042_NAME "max17042_battery"
...
#define FG_DATA_MODEL_70P  3
#define FG_DATA_MODEL_100P 4
#define NORMAL_TEMP_RNG_INDEX 1
...
static struct msic_batt_sfi_prop_oem1 *sfi_table_oem1;
...
/* Sysfs Entry for updationg the battery status */
static ssize_t get_battery_is_valid(struct device *dev,
				struct device_attribute *attr,
				char *buf);
static DEVICE_ATTR(is_valid, 0444, get_battery_is_valid, NULL);
...
/*
 * variables used for battery hardware revision
 */
char msic_hw_rev_txt_version[8];
unsigned char msic_hw_rev_txt_rev1;
unsigned char msic_hw_rev_txt_rev2;
					  ...
					  char *buf, loff_t off, size_t count)
{
	...
	int size = (BAT_EEPROM_SIZE  < count) ? BAT_EEPROM_SIZE : count;
	int retval = 0;

	if (mbi->is_eeprom_valid) {
		memcpy(buf, &sfi_table_oem1->EpromData, size);
		retval = size;
	}
	return retval;
}
				...
				char *buf, loff_t off, size_t count)
{
	...
	if (mbi->is_eeprom_valid) {
		memcpy(buf, &sfi_table->batt_id, size);
		retval = size;
	}
	return retval;
 ...
 * Returns FG curve ID
 */
static int get_batt_fg_curve_index(u8 model)
{
	...
	mip_offset = UMIP_BATT_FG_TABLE_OFFSET;
	for (i = 0; i < num_tbls; i++) {
		ret = intel_scu_ipc_read_mip(batt_id, BATTID_STR_LEN,
					     (mip_offset + BATT_FG_TBL_BATTID),
					     0);
		if (ret) {
			dev_warn(msic_dev, "%s: umip read failed\n", __func__);
			goto get_idx_failed;
		...
		ret = intel_scu_ipc_read_mip(&fg_tbl_type_id,
					     FG_TBL_TYPE_LEN,
					     (mip_offset + BATT_FG_TBL_TYPE),
					     0);
		if (ret) {
			dev_warn(msic_dev,
				 "%s: umip read of FG table type failed\n",
				 __func__);
			goto get_idx_failed;
		}

		if ((!strncmp(batt_id, sfi_table->batt_id, BATTID_STR_LEN)) &&
		    (fg_tbl_type_id == model))
			break;
	...
	if (i < num_tbls) {
		dev_info(msic_dev, "%d UMIP Battery ID: %s\n", i, batt_id);
		ret = i;
	} else {
		ret = -ENXIO;
	}
 ...
 */
static int intel_msic_store_refrenced_table(u8 model)
{
	...
	ret = intel_scu_ipc_read_mip(batt_id, BATTID_STR_LEN,
				     (mip_offset + BATT_FG_TBL_BATTID), 0);
	if (ret) {
		dev_warn(msic_dev, "%s: umip read failed\n", __func__);
		goto store_err;
	}

	ret = intel_scu_ipc_read_mip(&fg_tbl_type_id, FG_TBL_TYPE_LEN,
				     (mip_offset + BATT_FG_TBL_TYPE), 0);
	if (ret) {
		dev_warn(msic_dev, "%s: umip read failed\n", __func__);
		goto store_err;
	}
	...
	dev_info(msic_dev, "REF UMIP Battery ID: %s\n", batt_id);

	dev_info(msic_dev, "FG Data Model: %d\n", model);
	dev_info(msic_dev, "REF UMIP FG Table Type: %d\n", fg_tbl_type_id);

	/*if correct table is already in place then don't do anything*/
	if ((!strncmp(batt_id, sfi_table->batt_id, BATTID_STR_LEN)) &&
	    (fg_tbl_type_id == model)) {
		dev_info(msic_dev,
		 "%s: Correct value is stored already\n", __func__);
		return 0;
	}

	batt_index = get_batt_fg_curve_index(model);
	if (batt_index < 0) {
		dev_err(msic_dev,
...
int intel_msic_restore_config_data(const char *name, void *data, int len)
{
	...
	if (!strncmp(name, MAX17042_NAME, MAX17XXX_STR_LEN))
		model = FG_DATA_MODEL_70P;

	ret = intel_msic_store_refrenced_table(model);
	if (ret < 0) {
		dev_err(msic_dev, "%s failed to read fg data\n", __func__);
		return ret;
	}
...
static int adc_to_temp(uint16_t adc_val)
{
	...
	if (adc_val > MSIC_BTP_ADC_MAX) {
		dev_warn(msic_dev, "Temperature out of Range: %u"
			 " Adjusting to Max value %d\n", adc_val,
			 MSIC_BTP_ADC_MAX);
		return therm_curve_data[0][1];
	} else if (adc_val < MSIC_BTP_ADC_MIN) {
		dev_warn(msic_dev, "Temperature out of Range: %u"
			 " Adjusting to Min value %d\n", adc_val,
			 MSIC_BTP_ADC_MIN);
		return therm_curve_data[THERM_CURVE_MAX_SAMPLES-1][0];
	}
...
static void msic_batt_temp_charging(struct work_struct *work)
{
	...
	if (ret) {
		...
		mutex_lock(&mbi->batt_lock);
		if (cvref <
		    sfi_table->temp_mon_range[NORMAL_TEMP_RNG_INDEX].maint_chrg_vol_ul)
	...
	if (mbi->batt_props.health == POWER_SUPPLY_HEALTH_GOOD) {
		if (volt > max_volt) {
			mbi->batt_props.health =
				POWER_SUPPLY_HEALTH_OVERVOLTAGE;
			dev_info(msic_dev,
				 "Setting battery-health, overvoltage");
		}
...
#ifndef CONFIG_MMI_DISABLE_CHRG_LED
		/* Enable charger LED */
...
static unsigned char battery_is_cpyrght_vld(void)
{
	if (strncmp((const char *)
		...
		if (strncmp((const char *)
			(&sfi_table_oem1->EpromData)+CPYRGHT_STRING_PART_2_LEN,
...
static int sfi_table_populate_oem1(struct sfi_table_header *table)
{
	...
	totentrs = SFI_GET_NUM_ENTRIES(sb, struct msic_batt_sfi_prop_oem1);
	if (totentrs) {
		pentry = (struct msic_batt_sfi_prop_oem1 *)sb->pentry;
		totlen = totentrs * sizeof(*pentry);
		memcpy(sfi_table_oem1, pentry, totlen);
		if (battery_is_cpyrght_vld())
			mbi->is_eeprom_valid = true;
		else
			mbi->is_eeprom_valid = false;
	} else {
		dev_warn(msic_dev, "Invalid eeprom data\n");
		mbi->is_eeprom_valid = false;
	}
...
static u8 compute_pwrsrc_lmt_reg_val(int temp_high, int temp_low)
{
	...
	else if (temp_high >= 54)
		data |= CHR_PWRSRCLMT_TMPH_55;
	else if (temp_high >= 49)
...
static void init_batt_thresholds(struct msic_power_module_info *mbi)
{
	...
	data[1] = compute_pwrsrc_lmt_reg_val(batt_thrshlds->temp_high,
			batt_thrshlds->temp_low);
	dev_dbg(msic_dev, "PWRSRCLMT: 0x%x\n", data[1]);
...
static int init_msic_regs(struct msic_power_module_info *mbi)
{
	...
	static u8 data[] = {
		...
		WDTWRITE_UNLOCK_VALUE,
		(MSIC_CHRG_EXTCHRDIS | MSIC_EMRG_CHRG_ENBL |
		 MSIC_EMRG_CHRG_TEMP),
...
static int msic_battery_probe(struct ipc_device *ipcdev)
{
	...
	const char *revision_name = "SN95031";

	msic_hw_rev_txt_version[0] = 0;
	msic_hw_rev_txt_rev1 = 0;
	msic_hw_rev_txt_rev2 = 0;
	...
	sfi_table_oem1 = kzalloc(sizeof(struct msic_batt_sfi_prop_oem1),
				 GFP_KERNEL);
	if (!sfi_table_oem1) {
		dev_err(&ipcdev->dev, "%s(): memory allocation failed\n",
			__func__);
	...
	/* check for valid SFI table entry for OEM1 table */
	if (sfi_table_parse(SFI_SIG_OEM1, NULL, NULL,
			 sfi_table_populate_oem1)) {
		mbi->is_eeprom_valid = false;
	}
	...
	retval = sysfs_create_bin_file(&ipcdev->dev.kobj,
						&mmi_battery_eeprom_attr);
	if (retval) {
		pr_err("Failed to create eeprom.bin attribute : %d\n", retval);
	...
	if (retval) {
		pr_err("Failed to create uid.bin attribute : %d\n", retval);
		goto error_create_uid_file;
	}

	retval = device_create_file(&ipcdev->dev, &dev_attr_is_valid);
	if (retval < 0) {
		pr_err("Failed to create is_valid attribute : %d\n", retval);
		goto error_create_is_valid;
	}
	...
	/*
	* Now read the MSIC version details from reg 0 and reg 1
	*/
	msic_hw_rev_txt_version[0] = 0;
	msic_hw_rev_txt_rev1 = 0;
	msic_hw_rev_txt_rev2 = 0;
	...
	dev_info(msic_dev, "ID0 = 0x%x, retval=%d", msic_hw_rev_txt_rev1,
			retval);
	retval = intel_scu_ipc_ioread8(0x001,  &msic_hw_rev_txt_rev2);
	dev_info(msic_dev, "ID1 = 0x%xi, retval=%d", msic_hw_rev_txt_rev2,
			retval);
...
error_create_is_valid:
	device_remove_file(&ipcdev->dev, &mmi_battery_uid_attr);
error_create_uid_file:
	device_remove_file(&ipcdev->dev, &mmi_battery_eeprom_attr);
...
late_initcall(msic_battery_module_init);
