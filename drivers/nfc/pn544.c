...
#if (1)
	pr_info("%s : set ven and firm to low\n", __func__);
	/* Hard code Polarity to High to use same drivers
	with SMi HW which has additional CAP. */
	pn544_dev->nfc_enable = 1;
	/* disable fw download */
...
static long int pn544_dev_ioctl(struct file *filp,
		...
		const struct i2c_device_id *id)
{
	...
	if (unlikely(enable_irq_wake(client->irq))) {
