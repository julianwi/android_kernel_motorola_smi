...
/* Start - MOT changes */
#define SYSTHERM0H		0x1DC
#define SYSTHERM0L		0x1DD

#define SYSTHERM1H		0x1DE
#define SYSTHERM1L		0x1DF

#define SYSTHERM2H		0x1E0
#define SYSTHERM2L		0x1E1

#define BATT_TEMPH		0x1D6
#define BATT_TEMPL		0x1D7

#define RTCSTATUS		0x154
...
void *intel_mid_gpadc_alloc(int count, ...)
{
	...
	if (!mgi->initialized) {
		dev_err(mgi->dev, "!mgi->initialized\n");
		return NULL;
	}
...
static ssize_t mot_intel_mid_gpadc_show_rtcstatus(struct device *dev,
				struct device_attribute *attr, char *buf)
{

	u8 data;
	struct gpadc_info *mgi = &gpadc_info;
	gpadc_read(RTCSTATUS, &data);
...
static ssize_t mot_intel_mid_gpadc_show_batt_temph(struct device *dev,
				struct device_attribute *attr, char *buf)
{

	u8 data;
	struct gpadc_info *mgi = &gpadc_info;
	gpadc_read(BATT_TEMPH, &data);

	return sprintf(buf, "%x", data);

}

static ssize_t mot_intel_mid_gpadc_show_batt_templ(struct device *dev,
				struct device_attribute *attr, char *buf)
{

	u8 data;
	struct gpadc_info *mgi = &gpadc_info;
	gpadc_read(BATT_TEMPL, &data);
	return sprintf(buf, "%x", data);
}


static ssize_t mot_intel_mid_gpadc_show_systherm0h(struct device *dev,
				struct device_attribute *attr, char *buf)
{

	u8 data;
	struct gpadc_info *mgi = &gpadc_info;
	gpadc_read(SYSTHERM0H, &data);

	return sprintf(buf, "%x", data);
}

static ssize_t mot_intel_mid_gpadc_show_systherm0l(struct device *dev,
				struct device_attribute *attr, char *buf)
{

	u8 data;
	struct gpadc_info *mgi = &gpadc_info;
	gpadc_read(SYSTHERM0L, &data);
	return sprintf(buf, "%x", data);
}

static ssize_t mot_intel_mid_gpadc_show_systherm1h(struct device *dev,
				struct device_attribute *attr, char *buf)
{

	u8 data;
	struct gpadc_info *mgi = &gpadc_info;
	gpadc_read(SYSTHERM1H, &data);
	return sprintf(buf, "%x", data);
}

static ssize_t mot_intel_mid_gpadc_show_systherm1l(struct device *dev,
				struct device_attribute *attr, char *buf)
{

	u8 data;
	struct gpadc_info *mgi = &gpadc_info;
	gpadc_read(SYSTHERM1L, &data);
	return sprintf(buf, "%x", data);
}

static ssize_t mot_intel_mid_gpadc_show_systherm2h(struct device *dev,
				struct device_attribute *attr, char *buf)
{

	u8 data;
	struct gpadc_info *mgi = &gpadc_info;
	gpadc_read(SYSTHERM2H, &data);
	return sprintf(buf, "%x", data);
}

static ssize_t mot_intel_mid_gpadc_show_systherm2l(struct device *dev,
				struct device_attribute *attr, char *buf)
{

	u8 data;
	struct gpadc_info *mgi = &gpadc_info;
	gpadc_read(SYSTHERM2L, &data);
	return sprintf(buf, "%x", data);
}
...
/* MOT Changes */
static DEVICE_ATTR(systherm0h, S_IRUSR | S_IRGRP | S_IROTH,
		mot_intel_mid_gpadc_show_systherm0h, NULL);
static DEVICE_ATTR(systherm0l, S_IRUSR | S_IRGRP | S_IROTH,
		mot_intel_mid_gpadc_show_systherm0l, NULL);


static DEVICE_ATTR(systherm1h, S_IRUSR | S_IRGRP | S_IROTH,
		mot_intel_mid_gpadc_show_systherm1h, NULL);
static DEVICE_ATTR(systherm1l, S_IRUSR | S_IRGRP | S_IROTH,
		mot_intel_mid_gpadc_show_systherm1l, NULL);

static DEVICE_ATTR(systherm2h, S_IRUSR | S_IRGRP | S_IROTH,
		mot_intel_mid_gpadc_show_systherm2h, NULL);
static DEVICE_ATTR(systherm2l, S_IRUSR | S_IRGRP | S_IROTH,
		mot_intel_mid_gpadc_show_systherm2l, NULL);

static DEVICE_ATTR(batt_temph, S_IRUSR | S_IRGRP | S_IROTH,
		mot_intel_mid_gpadc_show_batt_temph, NULL);

static DEVICE_ATTR(batt_templ, S_IRUSR | S_IRGRP | S_IROTH,
		mot_intel_mid_gpadc_show_batt_templ, NULL);

static DEVICE_ATTR(rtcstatus, S_IRUSR | S_IRGRP | S_IROTH,
		mot_intel_mid_gpadc_show_rtcstatus, NULL);
...
static struct attribute *intel_mid_gpadc_attrs[] = {
	...
	&dev_attr_systherm0h.attr,
	&dev_attr_systherm0l.attr,
	&dev_attr_systherm1h.attr,
	&dev_attr_systherm1l.attr,
	&dev_attr_systherm2h.attr,
	&dev_attr_systherm2l.attr,
	&dev_attr_batt_temph.attr,
	&dev_attr_batt_templ.attr,
	&dev_attr_rtcstatus.attr,
	NULL,
};
...
static int msic_gpadc_probe(struct ipc_device *ipcdev)
{
	struct gpadc_info *mgi = &gpadc_info;
...
static int msic_gpadc_remove(struct ipc_device *ipcdev)
{
	struct gpadc_info *mgi = &gpadc_info;
