...
static int penwell_otg_set_power(struct usb_phy *otg,
				unsigned mA)
{
	struct penwell_otg		*pnw = the_transceiver;
	unsigned long			flags;
...
static void penwell_otg_change_output_vol_active_mode(void)
{
	struct penwell_otg		*pnw = the_transceiver;
	struct intel_mid_otg_xceiv	*iotg = &pnw->iotg;
	int				retval;
	...
	if (!is_clovertrail(to_pci_dev(pnw->dev))) {
		retval = penwell_otg_ulpi_write(iotg, ULPI_VS3CLR, REG3V3_VSEL);
		if (retval)
			dev_warn(pnw->dev,
				"change output voltage active mode failure\n");
	}
}
...
static void penwell_otg_work(struct work_struct *work)
{
	...
	case OTG_STATE_B_IDLE:
		if (hsm->id == ID_A || hsm->id == ID_ACA_A) {
			...
			penwell_otg_change_output_vol_active_mode();
			...
			if (hsm->b_ssend_srp && hsm->b_se0_srp) {
				...
				if (pnw->iotg.otg.otg->start_srp)
					pnw->iotg.otg.otg->start_srp(pnw->iotg.otg.otg);
	...
	case OTG_STATE_A_HOST:
		if (hsm->id == ID_B || hsm->id == ID_ACA_B || hsm->a_bus_drop) {
			...
			wake_lock(&pnw->wake_lock);
			iotg->otg.state = OTG_STATE_A_VBUS_ERR;
		} else if (!hsm->a_bus_req && iotg->otg.otg->host->b_hnp_enable) {
	...
	case OTG_STATE_A_SUSPEND:
		if (hsm->id == ID_B || hsm->id == ID_ACA_B ||
			...
			otg_set_vbus(iotg->otg.otg, false);
			iotg->otg.state = OTG_STATE_A_VBUS_ERR;
		} else if (!hsm->b_conn && !pnw->iotg.otg.otg->host->b_hnp_enable) {
			/* Move to A_WAIT_BCON */
