...
int gpsdrv_open(struct inode *inod, struct file *file)
{
	...
	if (ret == -EINPROGRESS) {
		...
		if (0 == timeout) {
			...
			GPSDRV_ERR("GPS Device Registration Failed-ST "
				"Reg CB called with ivalid value %d",
				hgps->streg_cbdata);
			clear_bit(GPS_ST_REGISTERED, &hgps->state);
			return -EAGAIN;
		}
	...
	tasklet_init(&hgps->gpsdrv_tx_tsklet,
		(void *) gpsdrv_tsklet_write,
		(unsigned long) hgps);
 ...
 * gpsdrv_power_down()
 */
int gpsdrv_power_down(struct gpsdrv_data *hgps)
{
	unsigned char power_cmd[] = {0x09, 0x00, 0x0c, 0x00, 0x10,
		0x00, 0xfb, 0x03, 0x00, 0x17, 0x00, 0x00,
		0x25, 0x01, 0x10, 0x03};
	struct sk_buff *pwr_dwn_skb;

	printk(KERN_ERR"powering down GPS IP now...\n");
	pwr_dwn_skb = alloc_skb(sizeof(power_cmd), GFP_KERNEL);
	if (!pwr_dwn_skb)
		return -ENOMEM;

	memcpy(skb_put(pwr_dwn_skb, sizeof(power_cmd)), power_cmd,
			sizeof(power_cmd));

	hgps->st_write(pwr_dwn_skb);
	msleep(250);
	return 0;
}
...
int gpsdrv_release(struct inode *inod, struct file *file)
{
	...
	if (gpsdrv_power_down(hgps))
		GPSDRV_ERR("power down failed");
 ...
 *           else suitable error code
 */
ssize_t gpsdrv_read(struct file *file,
	char __user *data,
	size_t size,
	loff_t *offset)
{
 ...
 *           else suitable error code
 */
ssize_t gpsdrv_write(struct file *file,
	...
	loff_t *offset)
{
	...
	memcpy(skb_put(skb, GPS_CH9_PKT_HDR_SIZE - 1),
		&gpsdrv_hdr,
	...
	if (0 != hgps->tx_count) {
		...
		if ((0 != hgps->tx_count) &&
			(!skb_queue_empty(&hgps->tx_list))) {
			/* Schedule the Tx-task let */
			GPSDRV_VER(" Scheduling tasklet to write in "
				"gpsdrv_write");
			tasklet_schedule(&hgps->gpsdrv_tx_tsklet);
		}
	}
 ...
 *           else suitable error code
 */
static long gpsdrv_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	struct sk_buff *skb = NULL;
	...
	case FIONREAD:
		 ...
		 * And update the argument with skb->len to provide amount of
		 * data available in the available SKB
		 */
		skb = skb_dequeue(&hgps->rx_list);
		if (skb != NULL) {
 ...
 */
const struct file_operations gpsdrv_chrdev_ops = { .owner = THIS_MODULE,
	.open = gpsdrv_open,
	.read = gpsdrv_read,
	.write = gpsdrv_write,
