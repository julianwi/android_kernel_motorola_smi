...
extern struct platform_driver smd_qhd_amoled_430_driver;
#endif
...
static void intel_dbi_fifo_query_timer_init(struct drm_device *dev)
{
	struct drm_psb_private *dev_priv = dev->dev_private;
	unsigned long flags;
	...
	spin_lock_init(&dev_priv->dbi_fifo_query_timer_lock);
	spin_lock_irqsave(&dev_priv->dbi_fifo_query_timer_lock, flags);

	hrtimer_init(&dev_priv->dbi_fifo_query_hrtimer, CLOCK_MONOTONIC,
		HRTIMER_MODE_REL);
	dev_priv->dbi_fifo_query_hrtimer.function =
	intel_dbi_fifo_query_timer_func;
...
static int psb_driver_load(struct drm_device *dev, unsigned long chipset)
{
	...
	*/
	...
	if (ret)
		goto out_err;
	init_waitqueue_head(&dev_priv->dbi_fifo_query_queue);
	atomic_set(&dev_priv->dbi_fifo_query, 0);

	intel_dbi_fifo_query_timer_init(dev);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,8,0)
#pragma message "content of LINUX_VERSION_CODE: " STR(LINUX_VERSION_CODE)
#pragma message "content of KERNEL_VERSION(3,8,0): " STR(KERNEL_VERSION(3,8,0))
...
#ifdef CONFIG_SUPPORT_SMD_QHD_AMOLED_COMMAND_MODE_DISPLAY
	ret = platform_driver_register(&smd_qhd_amoled_430_driver);
	if (ret != 0) {
		return ret;
	}
#endif
