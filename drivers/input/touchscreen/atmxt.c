...
static int atmxt_gpio_init(struct atmxt_driver_data *dd)
{
	...
	struct gpio touch_gpio[] = {
		...
		{dd->pdata->gpio_irq,   GPIOF_IN,           "touch_irq"},
	};
	...
	dd->client->irq = gpio_to_irq(dd->pdata->gpio_irq);
...
static int atmxt_request_irq(struct atmxt_driver_data *dd)
{
	...
	err = gpio_get_value(dd->pdata->gpio_irq);
	if (err < 0) {
...
atmxt_force_bootloader_check_reset:
	...
	if (!chg_used) {
		for (i = 0; i < 128; i++) {
			if (gpio_get_value(dd->pdata->gpio_irq) == 1)
				break;
			else
				udelay(2000);
		}
...
static bool atmxt_wait4irq(struct atmxt_driver_data *dd)
{
	...
	for (i = 0; i < 500; i++) {
		if (gpio_get_value(dd->pdata->gpio_irq) != 0) {
			msleep(20);
		} else {
			irq_low = true;
			break;
