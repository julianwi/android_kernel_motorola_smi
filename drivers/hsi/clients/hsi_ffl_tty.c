...
static void ffl_port_event(struct hsi_client *cl, unsigned long event)
{
	switch (event) {
 ...
 */
static void _ffl_forward_tty(struct tty_port *port, struct tty_struct *tty,
			     struct ffl_xfer_ctx	*ctx,
			     unsigned long		 *flags)
{
	struct hsi_msg		*frame;
...
static void ffl_do_tty_forward(struct work_struct *work)
{
	...
	if (tty) {
		...
		_ffl_forward_tty(&ctx->tty_prt, tty, &ctx->rx, &flags);
...
static int ffl_tty_port_activate(struct tty_port *port, struct tty_struct *tty)
{
	...
	err = hsi_register_port_event(ctx->client, ffl_port_event);
	if (err < 0) {
		pr_err(DRVNAME ":Register HSI port event failed (%d)\n", err);
...
static void ffl_tty_port_shutdown(struct tty_port *port)
{
	...
	flush_work(&ctx->start_tx);
	flush_work(&ctx->stop_tx);
...
static int ffl_driver_probe(struct device *dev)
{
	...
	if (unlikely(!tty_port_register_device(tty_prt, ffl_drv.tty_drv,
			l, dev))) {
		err = -EFAULT;
