...
#define AKM8963_DRDY_SLEEP	10
#define AKM8963_DRDY_MAX_WAIT	11
	...
	int size)
{
	...
	while (gpio_get_value(akm->gpio) == 0) {
		wait_count++;
		if (wait_count > AKM8963_DRDY_MAX_WAIT) {
			dev_err(&akm->i2c->dev,
				"%s: wait time exceeded.\n", __func__);
			return -1;
		}
		msleep(AKM8963_DRDY_SLEEP);
	}
	...
	if (err < 0) {
		dev_err(&akm->i2c->dev, "%s failed.\n", __func__);
		return -1;
	}
	/* Check ST bit */
	if ((buffer[0] & 0x01) != 0x01) {
		dev_err(&akm->i2c->dev, "%s ST is not set.\n", __func__);
		return -1;
	}
