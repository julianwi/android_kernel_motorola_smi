...
static int atomisp_stop_flash(struct atomisp_device *isp)
{
	...
	ctrl.id = V4L2_CID_FLASH_STROBE_COMPAT;
	ctrl.value = 0;
...
static int atomisp_start_flash(struct atomisp_device *isp)
{
	...
	ctrl.id = V4L2_CID_FLASH_TIMEOUT_COMPAT;
...
static int atomisp_wdt_pet_dog(struct atomisp_device *isp)
{
    const unsigned long timeout =
            ...
            msecs_to_jiffies(ATOMISP_WDT_TIMEOUT_VIDEO) :
            msecs_to_jiffies(ATOMISP_WDT_TIMEOUT)) + jiffies;
	isp->timeout_cnt = 0;
...
void atomisp_work(struct work_struct *work)
{
	...
	for (;;) {
		...
		if (flash_in_progress &&
			 ...
			 * correctly exposed. */
			if (atomic_read(&flash_sequence) < atomic_read(
						&isp->sequence))
				isp->fr_status =
		...
		if (!flash_enabled && isp->params.num_flash_frames) {
			...
			atomic_set(&flash_sequence,
					atomic_read(&isp->sequence));
			if (ret)
				isp->fr_status =
