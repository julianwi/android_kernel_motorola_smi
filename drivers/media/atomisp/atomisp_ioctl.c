	...
	struct v4l2_ext_controls *c)
{
	...
	for (i = 0; i < c->count; i++) {
		...
		case V4L2_CID_FLASH_INTENSITY_COMPAT:
		case V4L2_CID_FLASH_TORCH_INTENSITY_COMPAT:
		case V4L2_CID_FLASH_INDICATOR_INTENSITY_COMPAT:
		case V4L2_CID_FLASH_TIMEOUT_COMPAT:
		case V4L2_CID_FLASH_STROBE_COMPAT:
	...
	struct v4l2_ext_controls *c)
{
	...
	for (i = 0; i < c->count; i++) {
		...
		case V4L2_CID_TEST_PATTERN_COMPAT:
			ret = v4l2_subdev_call(
				isp->inputs[isp->input_curr].camera,
				core, s_ctrl, &ctrl);
			break;
	...
	bool valid_prio, unsigned int cmd, void *arg)
{
	...
	pr_debug("atomisp_vidioc_default cmd=%x\n", cmd);
