...
{
	*val = (MT9E013_FOCAL_LENGTH_NUM << 16) | MT9E013_FOCAL_LENGTH_DEN;
	return 0;
}
...
static int mt9e013_g_fnumber(struct v4l2_subdev *sd, s32 *val)
{
	...
	*val = (MT9E013_F_NUMBER_NUM << 16) | MT9E013_F_NUMBER_DEN;
	return 0;
}

static int mt9e013_g_fnumber_range(struct v4l2_subdev *sd, s32 *val)
{
	*val = ((MT9E013_F_NUMBER_NUM & 0xff) << 24) |
		(MT9E013_F_NUMBER_DEN << 16) |
		(MT9E013_F_NUMBER_NUM << 8) | MT9E013_F_NUMBER_DEN;
	return 0;
}
...
static struct mt9e013_control mt9e013_controls[] = {
	...
	{
		.qc = {
			.id = V4L2_CID_TEST_PATTERN_COMPAT,
			.type = V4L2_CTRL_TYPE_INTEGER,
			.name = "Test pattern",
			.minimum = 0,
			.maximum = 0xffff,
...
#ifdef CONFIG_VIDEO_MT9E013_ENZO_DISABLE_AND_REVISIT
/* TBD TBD: For now, disable this and revisit later,
	when binning mode for 1080p is fully implemented */
	/*
			 ...
			 const struct i2c_device_id *id)
{
	...
	v4l2_info(client, "%s: driver successfully probed using setting: %s\n",
		  __func__, MT9E013_SETTING_NAME);
