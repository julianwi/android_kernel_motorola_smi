...
void sn95031_configure_pll(struct snd_soc_codec *codec, int operation)
{
	...
	if (ctx->pll_state == PLL_ENABLE_PENDING
		...
		/*work around VIHF vibe issue by leaving PLL enabled:
		  current drain is negligible when Penwell clock and
		  VAUD regulator are disabled
		snd_soc_write(codec, SN95031_AUDPLLCTRL, 0);*/
		    ...
		    struct snd_kcontrol *kcontrol, int event)
{
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		...
		/* power up the rail */
		snd_soc_write(w->codec, SN95031_VIHF, 0xF6);
		usleep_range(1000, 1100);
	} else if (SND_SOC_DAPM_EVENT_OFF(event)) {
		pr_debug("VIHF SND_SOC_DAPM_EVENT_OFF doing rail shutdown\n");
		/*use hard-off here because auto mode leaks 300uA*/
		snd_soc_write(w->codec, SN95031_VIHF, 0xE4);
	}
	return 0;
}
...
static const char *sn95031_bool_off_on_text[] = {"Off", "On"};
static const struct soc_enum sn95031_vibra1_start_enum =
	SOC_ENUM_SINGLE(SN95031_VIB1C1, 2, 2, sn95031_bool_off_on_text);
static const struct soc_enum sn95031_vibra2_start_enum =
	SOC_ENUM_SINGLE(SN95031_VIB2C1, 2, 2, sn95031_bool_off_on_text);
static const struct soc_enum sn95031_vibra1_brake_enum =
	SOC_ENUM_SINGLE(SN95031_VIB1C1, 1, 2, sn95031_bool_off_on_text);
static const struct soc_enum sn95031_vibra2_brake_enum =
	SOC_ENUM_SINGLE(SN95031_VIB2C1, 1, 2, sn95031_bool_off_on_text);

static const struct soc_enum sn95031_sidetone_enable_enum =
	SOC_ENUM_SINGLE(SN95031_VOICETXCTRL, 0, 2, sn95031_bool_off_on_text);
static const struct soc_enum sn95031_voicetx_enable_enum =
	SOC_ENUM_SINGLE(SN95031_VOICETXCTRL, 2, 2, sn95031_bool_off_on_text);
static const struct soc_enum sn95031_voicetx_hpf_enable_enum =
	SOC_ENUM_SINGLE(SN95031_VOICETXCTRL, 5, 2, sn95031_bool_off_on_text);

static const char *sn95031_sinc45_text[] = {"SINC4", "SINC5"};
static const struct soc_enum sn95031_voicepath_sinc45_enum =
	SOC_ENUM_SINGLE(SN95031_VOICETXCTRL, 1, 2, sn95031_sinc45_text);
...
static const char *sn95031_amic1_select_text[] = {"Internal", "Headset"};
static const struct soc_enum sn95031_amic1_select_enum =
		SOC_ENUM_SINGLE_EXT(2, sn95031_amic1_select_text);
static unsigned int sn95031_amic1_select;
static int sn95031_set_amic1_select(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	u8 select = ucontrol->value.integer.value[0];

	if (select == sn95031_amic1_select)
		return 0;
	if (select) {
		pr_debug("Routing headset to AMIC1\n");
		intel_scu_ipc_iowrite8(0x70, 0x31);
	} else {
		pr_debug("Routing internal mic to AMIC1\n");
		intel_scu_ipc_iowrite8(0x70, 0x30);
	}
	sn95031_amic1_select = select;
	return 0;
}
static int sn95031_get_amic1_select(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = sn95031_amic1_select;
	return 0;
}
...
static const struct snd_kcontrol_new sn95031_snd_controls[] = {
	...
	/*add controls for sidetone mixer*/
	SOC_ENUM("Voice TX Switch", sn95031_voicetx_enable_enum),
	SOC_SINGLE("Voice TX Volume", SN95031_VOICETXVOL, 0, 0x47, 0),
	SOC_ENUM("Voice HPF", sn95031_voicetx_hpf_enable_enum),
	SOC_ENUM("Voice Sinc Filter", sn95031_voicepath_sinc45_enum),
	SOC_ENUM("Sidetone Switch", sn95031_sidetone_enable_enum),
	SOC_SINGLE("Sidetone Volume", SN95031_SIDETONEVOL, 0, 0x3E, 0),
	/*add control for amic1 mux on SMi*/
	SOC_ENUM_EXT("AMIC1 Select", sn95031_amic1_select_enum,
			sn95031_get_amic1_select, sn95031_set_amic1_select),
};
...
static const struct snd_soc_dapm_route sn95031_audio_map[] = {
	...
	/*on SMi, both internal mics are powered by AMIC2Bias.
	  headset driver controls AMIC1Bias manually with
	  mfld_jack_[en|dis]able_mic_bias() in machine driver. */
	...
	/*starting with SMi P3, analog mics use DMIC paths
	  via external ADC*/
	{ "DMIC1", NULL, "AMIC2Bias"},
	{ "DMIC5", NULL, "AMIC2Bias"},
...
static int sn95031_codec_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_write(codec, SN95031_VAUD, 0x24);
	/*use hard-off for VIHF because auto mode leaks ~300uA*/
	snd_soc_write(codec, SN95031_VIHF, 0xE4);
	...
	/*work around VIHF vibe issue by leaving PLL enabled:
	  current drain is negligible when Penwell clock and
	  VAUD regulator are disabled */
	snd_soc_write(codec, SN95031_AUDPLLCTRL, 0x22);
